...
class AdminImportControllerCore extends AdminController
{
	...
	public static $validators = array(
		...
		'desoferta' => array('AdminImportController', 'createMultiLangField'),
	);
		...
		switch ((int)Tools::getValue('entity'))
		{
			...
			case $this->entities[$this->l('Products')]:
				...
				$this->available_fields = array(
					...
					'desoferta' => array('label' => $this->l('Descripcion oferta')
					),
				);
	...
	public function productImport()
	{
		...
		for ($current_line = 0; $line = fgetcsv($handle, MAX_LINE_SIZE, $this->separator); $current_line++)
		{
			...
			else
			{
				...
				// MGM 26/01/16 AÑADIDO PARA COMPROBAR SI ANTES DE IMPORTAR EL ARTÍCULO TIENE PRECIO ESPECÍFICO. SI ES ASÍ
			    // SE BORRA PARA QUE PUEDA CREARSE EL NUEVO O SI NO TIENE, QUE EN LA FICHA DE PRODUCTO NO APAREZCA EL ANTERIOR.
				...
				if (is_array($specific_price) && isset($specific_price['id_specific_price'])) {
					Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'specific_price WHERE id_specific_price='.$specific_price['id_specific_price']);
				}
