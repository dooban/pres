 ...
 -->                                </div>
                                <div class="btn_action">
                                    ...
                                    {if $wishlists|count > 1}
                                        ...
                                        {/foreach}
                                    {/if}
<a class="exclusive button ajax_add_to_cart_button add-to-cart-in-wl" href="{$link->getPageLink('cart', true, NULL, "qty={$product.quantity|intval}&amp;id_product={$product.id_product|intval}&amp;add")|escape:'html':'UTF-8'}" data-id-attribute="{$product.id_product_attribute}" data-id-product="{$product.id_product|intval}" data-minimal_quantity="{$product.quantity|intval}" title="{l s='Add to cart' mod='blockwishlist'}"><span>{l s='Añadir al carrito' mod='blockwishlist'}</span></a>
                                </div>
                            </div>
                        </div>
                    </div>
