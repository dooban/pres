...
<table class="product" width="100%" cellpadding="4" cellspacing="0">
	...
	<tr>
		...
		<th class="product header small" width="40%" align="left">{l s='Product' pdf='true'}</th>
		<th class="product header small" width="6%">{l s='Tax Rate' pdf='true'}</th>

		<!-- {if isset($layout.before_discount)} -->
			<th class="product header-right small" width="10%">{l s='Base price' pdf='true'}<!--  {l s='(Tax excl.)' pdf='true'} --></th>
			<th class="product header-right small" width="8%">Dcto</th>
		<!-- {/if} -->

		<th class="product header-right small" width="10%">{l s='Unit Price' pdf='true'}<!--  {l s='(Tax excl.)' pdf='true'} --></th>
		<th class="product header-right small" width="6%">{l s='Qty' pdf='true'}</th>
		<th class="product header-right small" width="10%">{l s='Total' pdf='true'}<!--  {l s='(Tax excl.)' pdf='true'} --></th>
	</tr>
	</thead>
	...
	{foreach $order_details as $order_detail}
		...
		<tr class="product {$bgcolor_class}">
			...
			<td class="product left">
				...
				{else}
					{$order_detail.product_name}
					{if isset($order_detail.product_desoferta)}
						<br><small>{$order_detail.product_desoferta}</small>
					{/if}
				{/if}

			</td>
			<td class="product center">
				{str_replace("%", "", $order_detail.order_detail_tax_label)}
			</td>

			{if isset($layout.before_discount)}
				<td class="product right">
					{if isset($order_detail.product_price) && ($order_detail.product_price != $order_detail.unit_price_tax_excl_including_ecotax)}
...
<!-- 										<td style="width: 30%;">
						...
						{if isset($customization.datas[$smarty.const._CUSTOMIZE_FILE_]) && count($customization.datas[$smarty.const._CUSTOMIZE_FILE_]) > 0}
							<table style="width: 100%;">
								<tr>
									<td style="width: 100%;">{l s='image(s):' pdf='true'}</td>
