	...
	<div id="order-detail-content" class="table_block table-responsive">
		<table id="cart_summary" class="table table-bordered {if $PS_STOCK_MANAGEMENT}stock-management-on{else}stock-management-off{/if}">
			...
			<tfoot>
				...
				{*MGM 22/10/15 AÑADIDO PARA VER EL DESCUENTO TOTAL DEL PEDIDO *}
				...
				{if !(($totalReductonPercent-$total_discounts_negative) == 0)}
					<tr class="cart_total_discount">
					    <td colspan="5" class="text-right">{l s='Ahorro total del pedido (IVA no incluido)'}</td>
					    <td colspan="4" class="price" id="total_discount">
	                        <FORM>
	                        <INPUT TYPE="button" onClick="history.go(0)" VALUE="-{convertPrice price=$totalReductonValue - $total_products - $total_discounts_negative}">
	                        </FORM>
					        <!-- <span class="price-percent-reduction small">-{($totalReductonPercent-$total_discounts_negative)|round|string_format:"%d"}%</span><br> -->
					    </td>
					</tr>
				{/if}
				{*MGM 22/10/15 AÑADIDO PARA VER EL DESCUENTO TOTAL DEL PEDIDO FIN *}
