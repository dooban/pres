...
<div itemscope itemtype="http://schema.org/Product">
	...
	<div class="primary_block row">
		...
		<div class="pb-center-column col-xs-12 col-sm-4">
            ...
            <!-- MGM 03/11/15 AÑADIDO FABRICANTE DEL PRODUCTO -->
			<p id="product_manufacturer" {if $product->id_manufacturer == 0}style="display: none;"{/if}>
			<label for="product_reference">{l s='Manufacturer:'} </label>
            ...
            <!-- MGM 03/11/15 SE QUITA LA CONDICION DEL PRODUCTO DE LA FICHA
			...
			<form id="buy_block"{if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0} class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
				...
				<div class="box-info-product">
					<div class="content_prices clearfix">
						{if $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}
							...
							<div>
								...
								<p id="reduction_percent" {if !$product->specificPrice || $product->specificPrice.reduction_type != 'percentage'} style="display:none;"{/if}>{strip}
                                    ...
                                    <!-- MGM 03/11/15 MODIFICADO PARA QUE NO MUESTRE LOS DECIMALES EN LOS DESCUENTOS -->
										{if $product->specificPrice && $product->specificPrice.reduction_type == 'percentage'}-{($product->specificPrice.reduction*100)|@floor}%{/if}
                                    <!-- FIN MGM -->
									</span>
								{/strip}</p>
						...
						{if ($product->desoferta != '' && $product->desoferta != null && $product->on_sale)}
							<font color="#f13340">{$product->desoferta}</font>
						{/if}
					...
					<div class="box-cart-bottom">
						...
						<center>{if isset($HOOK_PRODUCT_ACTIONS) && $HOOK_PRODUCT_ACTIONS}{$HOOK_PRODUCT_ACTIONS}{/if}</center>
