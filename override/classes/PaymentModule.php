		...
		if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false)
		{
			...
			foreach ($order_detail_list as $key => $order_detail)
			{
				...
				if (!$order_creation_failed && isset($order->id))
				{
					...
					{
						Logger::addLog(implode("=",$product),1);
						...
						$product_var_tpl = array(
							...
							'customization' => array(),
							// MGM 29/10/15 SE AÑADEN ESTOS DOS CAMPOS PARA MOSTRARLOS EN EL EMAIL DE CONFIRMACIÓN DE PEDIDO
							'pricewt' =>  Tools::displayPrice($product['price_without_reduction']/(1+$product['rate']/100), $this->context->currency, false),
							'discount' => number_format((1-$price/($product['price_without_reduction']/(1+$product['rate']/100)))*100,2,",","."),
							'desoferta' => $product['desoferta']
							// MGM FIN
						);
			...
			// MGM 02/02/2016 METER LA ORDEN EN LA BASE DE DATOS DEL WISHLIST
	 		$db = DB::getInstance()->getValue('
	 		SELECT id_wishlist FROM '._DB_PREFIX_.'wishlist WHERE id_customer='.$order->id_customer.' AND name LIKE "COMPRADOS"');

	 		if (empty($db))
	 		{
	 			// NO EXISTE LA LISTA, ASÍ QUE LA CREAMOS
	 			DB::getInstance()->execute('
				INSERT INTO `'._DB_PREFIX_.'wishlist` (`id_customer`, `name`)
				VALUES ('.$order->id_customer.', "COMPRADOS")');
	 		}


			//OBTENEMOS LA ID DE LA WISHLIST DONDE VAMOS A INSERTAR LOS ARTÍCULOS
	 		$db = DB::getInstance()->getValue('
	 		SELECT id_wishlist FROM '._DB_PREFIX_.'wishlist WHERE id_customer='.$order->id_customer.' AND name LIKE "COMPRADOS"');

	 		DB::getInstance()->execute('
			INSERT IGNORE INTO `'._DB_PREFIX_.'wishlist_product` (`id_wishlist`, `id_product`, `id_product_attribute`, `quantity`, `priority`)
			(SELECT '.$db.', product_id, product_attribute_id, 1, 1 FROM '._DB_PREFIX_.'order_detail WHERE id_order='.$order->id.')'
	 		);
